      ...
      (function () {
        try {
          ...
          define('difference', function (other) {
            ...
            if (other && other[Symbol.iterator]) {
              for (const v of other) result.delete(v);
            }
            return result;
          });
          ...
          console.log('%c[SET POLYFILL] ✅ Métodos difference, intersection, union adicionados no MAIN WORLD', 'color: #00FF00; font-weight: bold;');
        } catch (e) {
          console.error('[SET POLYFILL] ❌ Erro ao aplicar polyfill:', e);
        }
      })();
    `, true);
  } catch (e) {
    console.error('[SET POLYFILL] ❌ Erro fatal ao injetar no main world:', e);
  }
})();
...
(() => {
    ...
    try {
      ...
      style.textContent = `
        ...
        .status {
          ...
          height: 8px;
          border-radius: 50%;
          background: ${navigator.onLine ? '#00d084' : '#e74c3c'};
        }
        .url {
          flex: 1;
          height: 28px;
  ...
  function applyLayoutAdjustment() {
    ...
    const isProblematicSite = isVectorizer || isMotionArray || isPlaceit || hostname.includes('canva');
    ...
    if (isPlaceit) {
      ...
      styleEl.textContent = `
        :root {
          --toolbar-offset: ${TITLE_BAR_HEIGHT}px;
        }
        html {
    ...
    shadowRoot.querySelector('.bar').addEventListener('click', (e) => {
      ...
      switch (action) {
        ...
        case 'reload':
          ...
          break;
        case 'downloads':
          shadowRoot.querySelector('.downloads-menu').classList.toggle('open');
          break;
        case 'minimize':
